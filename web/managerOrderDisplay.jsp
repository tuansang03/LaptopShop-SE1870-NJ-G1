        ...
        <div class="col-md-10 content">
            ...
            <div style="display: flex; align-items: center">
                <form style="width: 100%;" action="searchOrder" class="form-inline mb-3">
                    <input type="text" name="search" value="${search}" class="form-control mr-2" placeholder="Search Customer">
                ...
                <div style="display: flex">
                    ...
                    <Button name="action" value="wait" style="outline: none">
                        <div class="${action == 'wait' ? 'backgroundSelect': 'background'}" >
                            ...
                            <h6>Total Money: <fmt:formatNumber value="${totalAmountWait}" pattern="#,###" /></h6>
                        </div>
                    </Button>
                    <Button name="action" value="rejected" style="outline: none">
                        <div class="${action == 'rejected' ? 'backgroundSelect': 'background'}">
                            ...
                            <h6>Total Money: <fmt:formatNumber value="${totalAmountRejected}" pattern="#,###" /></h6>
                        </div>
                    </Button>
                    <Button name="action" value="accepted" style="outline: none">
                        <div class="${action == 'accepted' ? 'backgroundSelect': 'background'}">
                            ...
                            <h6>Total Money: <fmt:formatNumber value="${totalAmountAccepted}" pattern="#,###" /></h6>
                        </div>
                    </Button>
                    <Button name="action" value="intransit" style="outline: none">
                        <div class="${action == 'intransit' ? 'backgroundSelect': 'background'}">
                            ...
                            <h6>Total Money:<fmt:formatNumber value="${totalAmountIntransit}" pattern="#,###" /></h6>
                        </div>
                    </Button>
                    <Button name="action" value="failed" style="outline: none">
                        <div class="${action == 'failed' ? 'backgroundSelect': 'background'}">
                            ...
                            <h6>Total Money: <fmt:formatNumber value="${totalAmountFailed}" pattern="#,###" /></h6>
                        </div>
                    </Button>
                    <Button name="action" value="done" style="outline: none">
                        <div class="${action == 'done' ? 'backgroundSelect': 'background'}">
                            ...
                            <h6>Total Money: <fmt:formatNumber value="${totalAmountDone}" pattern="#,###" /></h6>
                        </div>
                    </Button>
                </div>
            </form>
                ...
                <tbody>
                    <c:forEach items="${listOrder}" var="o">
                        <tr>
                            ...
                            <td>
                                <c:if test="${o.getPaymentStatus() == null}"><span class="status-none">None</span></c:if>
                                <c:if test="${o.getPaymentStatus() != null}">
                                    <span class="${o.getPaymentStatus() == 'Success' ? 'status-success' : 'status-fail'}">
                                        ${o.getPaymentStatus()}
                                    </span>
                                </c:if>    
                            </td>
                            <td>
                                ...
                                <c:if test="${o.getVnPayTransactionId() != null}">${o.getVnPayTransactionId()}</c:if> 
                                </td>
                                <td>
